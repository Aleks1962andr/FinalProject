...
function CartDish() {
    ...
    const handleAddToOrder = (dish) => {
        setLoading(true);
        setTimeout(() => {
            dispatch(addToOrder({ ...dish, quantity: sum }));
            setLoading(false);
        }, 1000);
    };
            ...
            {data.filter((item) => item.title === title).map((elem, elemId) => (
                <div key={elemId} className="block">
                    ...
                    <div className="butCart">
                        <div className="butNavig">
                            <button className="butCartBackNav" onClick={() => navigate(-1)}>НАЗАД</button>
                        </div>
                        <div className="butMinus">
                            <button className="butCartBackDel" onClick={decrement}>
                                <img src="/image/minus.png" alt="icon" className="iconCart" />
                            </button>
                        </div>
                        <p className="sumCart">{sum}</p>
                        <div className="butPlus">
                            <button className="butCartBackPl" onClick={increment}>
                                <img src="/image/plus.png" alt="icon" className="iconCartpl" />
                            </button>
                        </div>
                        <div>
